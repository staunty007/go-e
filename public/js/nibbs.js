...
let transactionOtpCode,
	transactionMandateCode,
	amountToBePaid,
	bankCodeSelected = '070';

const loadBanks = () => {
	axios
		.get(`${baseUrl}/nibbs/all-banks`)
		.then((response) => {
...
const showOtpBox = (isOtp = false) => {
	document.getElementById('otp-details').style.display =
		isOtp ? 'block' :
		'none';
};

const showOtpPayment = (isOtp = false) => {
	document.getElementById('otp-payment').style.display =
		isOtp ? 'block' :
		'none';
};

const showSuccess = (isPaymentCompleted = false) => {
	document.getElementById('success-payment').style.display =
		isPaymentCompleted ? 'block' :
		'none';
};

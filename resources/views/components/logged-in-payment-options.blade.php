...
<div class="modal fade" tabindex="-1" role="dialog" style="" id="payment-options" style="display: block;background: rgba(0, 0, 0, 0.77);">
	<div class="modal-dialog " role="document">
		<div class="modal-content">
			<div class="modal-header headermodal text-center">
			...
			.then(result => {
				if (result.response.retn !== 0) {
					...
					fetch(`/ekedc/validate-payment/${accountType}/${meter_no}`)
						.then(res => res.json())
						.then(result => {
							// Check if we have no records for the payment validation
